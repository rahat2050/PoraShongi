...
function PremiumBack({ teacher, rounded }: { teacher: TeacherPublic; rounded: string }) {
  ...
  return (
    <div
      className={cn(
        "relative flex h-full flex-col overflow-hidden border-2 border-amber-300/90 bg-[linear-gradient(150deg,#042f2e_0%,#0f3d3a_55%,#4a2c0d_145%)] p-5 text-white shadow-[0_22px_60px_-24px_rgba(4,47,46,.7)] dark:border-amber-400/80",
        rounded,
      )}
    >
      <div className="pointer-events-none absolute -right-10 -top-12 h-36 w-36 rounded-full bg-amber-400/15 blur-2xl" aria-hidden />
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="inline-flex items-center gap-1 rounded-full bg-amber-400 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-slate-950">
          <Crown className="h-3 w-3" aria-hidden /> প্রিমিয়াম শিক্ষক
        </span>
        {teacher.verification_status === "verified" && (
          <span className="inline-flex items-center gap-1 rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-bold text-emerald-200">
            <BadgeCheck className="h-3 w-3" aria-hidden /> যাচাইকৃত
          </span>
        )}
      </div>

      <h3 className="mt-3 truncate text-lg font-black text-white">{name}</h3>
      ...
      <div className="mt-auto pt-3">
        <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[11px] leading-5 text-brand-50/90">
        ...
        <Link
          ...
          className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 text-sm font-bold text-slate-950 shadow-lg shadow-amber-900/30 transition-transform hover:-translate-y-0.5 hover:bg-amber-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900"
