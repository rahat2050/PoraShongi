...
export default async function ConversationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-8 sm:px-6">
      <Link href="/messages" className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-brand-700 dark:text-slate-300 dark:hover:text-brand-300">
        ...
        <CardContent className="p-0">
          <div className="flex items-center justify-between border-b border-slate-100 p-4 dark:border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{other?.role ? ROLE_LABELS[other.role].bn : "সদস্য"}</p>
              </div>
            </div>
          ...
          <div className="p-4">
            {blocked ? (
              ...
              <ChatPanel
                key={`${conversation.id}-${messagesResult.data?.at(-1)?.id ?? "empty"}`}
