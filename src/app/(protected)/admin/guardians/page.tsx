...
export const metadata: Metadata = { title: "Admin — Guardians" };
...
export default async function AdminGuardiansPage({
  ...
  const result = await adminListProfiles("guardian", page, PAGE_SIZE);
  const total = result.data?.total ?? 0;
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const buildHref = (p: number) =>
    `/admin/guardians${buildQueryString({ page: p > 1 ? p : undefined })}`;
          ...
          {result.error ? (
            ...
            <div className="p-6">
              <EmptyState
                ...
                description="Guardians will appear here once they sign up."
              />
            </div>
          )}
        </CardContent>
