...
export async function updateBaseProfile(input: {
  ...
  const parsed = baseProfileSchema.safeParse(input);
  if (!parsed.success) {
...
export async function updateTeacherProfile(input: unknown): Promise<ActionResult> {
  const profile = await requireProfile();
...
export async function updateGuardianProfile(input: unknown): Promise<ActionResult> {
  const profile = await requireProfile();
