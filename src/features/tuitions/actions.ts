...
async function guardianLinkedStudentId(profileId: string): Promise<string | null> {
...
export async function createTuition(
  ...
  if (profile.role === "guardian") {
    studentId = await guardianLinkedStudentId(profile.id);
  }
...
export async function updateTuition(
  ...
  if (!existing || existing.poster_id !== profile.id) {
    return failure("You can only edit your own tuition posts.");
  }
...
export async function setTuitionStatus(
  ...
  if (!existing || existing.poster_id !== profile.id) {
    return failure("You can only manage your own tuition posts.");
  }
...
export async function deleteTuition(tuitionId: string): Promise<ActionResult> {
  ...
  if (!existing || existing.poster_id !== profile.id) {
    return failure("You can only delete your own tuition posts.");
  }
